  ...
  setTimeout(() => {
    ...
    if (form) {
      ...
      form.addEventListener("submit", async (event) => {
        ...
        const bioInput = document.getElementById("registerBio") as HTMLTextAreaElement;
        ...
        if (!name) {
          ...
          if (formError) formError.textContent = "Name must be at least 2 characters long.";
          hasError = true;
        }
        ...
        if (!email) {
          ...
          if (formError) formError.textContent = "Email must be a valid @stud.noroff.no address.";
          hasError = true;
        }
          ...
          if (result.errors && result.errors.length > 0) {
            ...
            if (formError) {
              ...
              if (errorMessage.toLowerCase().includes("email") && errorMessage.toLowerCase().includes("exist")) {
                formError.textContent = "❌ An account with this email already exists. Try logging in instead.";
              } else if (errorMessage.toLowerCase().includes("email") && errorMessage.toLowerCase().includes("invalid")) {
                formError.textContent = "❌ Please enter a valid @stud.noroff.no email address.";
              } else if (errorMessage.toLowerCase().includes("password")) {
                formError.textContent = "❌ Password doesn't meet requirements. Use at least 8 characters.";
              } else if (errorMessage.toLowerCase().includes("name")) {
                formError.textContent = "❌ Please enter a valid name (at least 2 characters).";
              } else if (errorMessage.toLowerCase().includes("already")) {
                formError.textContent = "❌ This email is already registered. Please use the login page.";
              } else {
            ...
            if (formError) {
              ...
              formError.textContent = "✅ Account created successfully! Redirecting to login page...";
            }
    ...
    <div class="page active" id="registerPage">
        <div class="auth-container">
            <div class="auth-card">
                ...
                <div id="registerMessage" style="margin-bottom: 1rem; text-align: center; font-weight: 500;"></div>
